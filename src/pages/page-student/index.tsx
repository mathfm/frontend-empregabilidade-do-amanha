...
export function PageStudent() {
  ...
  useEffect(() => {
    async function loadRepositories() {
      ...
      const response = await axios.get(
        `https://api.github.com/users/${
          findUser.data.github_url
        }/repos?sort=created&direction=desc`
      );
    ...
    if (localStorage.getItem("id_student") !== null) {
      api
        .get(`/student/${localStorage.getItem("id_student")}`, {
          headers: { Authorization: "Bearer " + localStorage.getItem("token") },
        })
        .then((resp) => {
          setUser(resp.data);
          axios
            .get(
              `https://api.github.com/users/${resp.data.github_url || ""}/repos?sort=created&direction=desc`
            )
            .then((github) => {
  ...
  return (
    <section className="w-full min-h-screen flex flex-col md:flex-row gap-4 items-center justify-center bg-purple-950 p-4 md:p-8">
      {repository?.length !== undefined &&
