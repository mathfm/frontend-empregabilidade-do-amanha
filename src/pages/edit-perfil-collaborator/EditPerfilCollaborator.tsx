...
export default function EditPerfilCollaborator() {
  ...
  const updateCollaborator = async (data: FormData) => {
    try {
      await axios.put(
        `http://localhost:3000/api/employer/update/${tokenDecode.id}`,
        data
      );
      reset();
  ...
  return (
    <FormsCollaborator
      functionForms={updateCollaborator}
      methodPost="Atualizar"
